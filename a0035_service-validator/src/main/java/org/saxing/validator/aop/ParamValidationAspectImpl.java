...
public class ParamValidationAspectImpl {
    ...
    @Pointcut("@annotation(paramValidation)")
    public void annotationPointCutDefinition(final ParamValidation paramValidation) { }

    @Before("annotationPointCutDefinition(paramValidation)")
    public void valid(final JoinPoint joinPoint, final ParamValidation paramValidation) {

        ServiceValidationErrorCollection errors = new ServiceValidationErrorCollection();
        Object[] args = joinPoint.getArgs();

        if (paramValidation.nullSafe()) {
        ...
        if (paramValidation.javaxValidation()) {
            for (int argIndex = 0; argIndex < args.length; argIndex++) {
