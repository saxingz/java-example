 ...
 */
package org.saxing.netty.codec;
...
public class TestCodeC {
	...
	public NettyMessage getMessage() {
		...
		nettyMessageHeader.setAttachment(attachment);
	...
	public ByteBuf encode(NettyMessage msg) throws Exception {
		...
		sendBuf.writeInt((msg.getNettyMessageHeader().getAttachment().size()));
		String key = null;
		byte[] keyArray = null;
		Object value = null;

		for (Map.Entry<String, Object> param : msg.getNettyMessageHeader().getAttachment().entrySet()) {
			key = param.getKey();
	...
	public NettyMessage decode(ByteBuf in) throws Exception {
		...
		if (size > 0) {
			...
			keyArray = null;
			key = null;
			nettyMessageHeader.setAttachment(attch);
		}
